...
class CornersProblem(SearchProblem):
    ...
    def isGoal(self, state):
        ...
        if (x, y) in self.corners:
            level.add(self.corners.index((x, y)))
        if all(l in level for l in range(4)):
            return True
        return False
    ...
    def successorStates(self, state):
        ...
        for action in Directions.CARDINAL:
            ...
            if (not self.walls[nextx][nexty]):
                nextState = (nextx, nexty, level.copy())
                cost = 1
    ...
    def startingState(self):
        level = set()
        return (self.startingPosition + (level,))
...
def cornersHeuristic(state, problem):
    ...
    # *** Your Code Here ***
    x, y, level = state
    height = problem.walls.getHeight() - 3
    width = problem.walls.getWidth() - 3
    unexplored_corners = [
        corner for corner in problem.corners if problem.corners.index(corner) not in level]
    short_goal = min(distance.euclidean((x, y), corner)
                     for corner in unexplored_corners)
    result = short_goal

    for corner in unexplored_corners[1:]:
        result += min(height, width)
    # print(state,unexplored_corners,short_goal, result)
    return result
...
def foodHeuristic(state, problem):
    ...
    result = min(list(distance.manhattan(position, food)
                 for food in foods)) if len(foods) > 0 else 0
    distances = []
    for i in range(len(foods)):
        if len(foods) == 1:
            break
        dist = min(list(distance.manhattan(
            foods[i], food) for food in foods if food != foods[i]))
        distances.append(dist)
    distances_sorted = sorted(distances)
    result += sum(distances_sorted[1:]) if len(distances_sorted) > 0 else 0
    # print("n",state[0],foodGrid.asList(), result)
    return result
...
class AnyFoodSearchProblem(PositionSearchProblem):
    ...
    def __init__(self, gameState, start=None):
        ...
        self.goal = min(
            foods, key=lambda x: distance.manhattan(x, self.startState))
